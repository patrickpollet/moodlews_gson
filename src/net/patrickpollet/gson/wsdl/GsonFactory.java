...
public class GsonFactory extends JavaGeneratorFactory{
	
	
	 public GsonFactory(Emitter emitter) {
		 super (emitter);
	 }
	  ...
	  public Generator getGenerator(Binding binding, SymbolTable symbolTable) {
		  ...
		  if (include(binding.getQName())) {
	            Generator writer = new GsonBindingWriter(emitter, binding,
	                    symbolTable);
	            return writer;
		  }      
	  ...
	  public Generator getGenerator(TypeEntry type, SymbolTable symbolTable) {
		  if (include(type.getQName())) {
	            Generator writer = new GsonTypeWriter(emitter, type, symbolTable);
	            return writer;
	        }
	        else {
	            return new NoopGenerator();
