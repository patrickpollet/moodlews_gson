...
public class GsonStubWriter extends JavaStubWriter {
	...
	protected void writeHeaderComments(PrintWriter pw) throws IOException {
		...
		pw.println("/**");
		pw
				.println(" * Modified for Gsonlibrary by pp@patrickpollet.net using GsonStubWriter");
		pw.println(" */");
		pw.println();
	} // writeHeaderComments
	...
	public void writeConstructor(PrintWriter pw) throws IOException {
		...
		pw.println("    public " + this.className
				+ "(String service_url, String nameSpace, boolean debug) {");
	...
	protected void writeOperation(PrintWriter pw, BindingOperation operation,
		...
		String signature = parms.signature.replace(
		...
		pw
				.println("    MyHttpTransportSE httpTransport = this.makeHttpTransport();");

		pw.println("     try {");
		pw
				.println("	MyRestSerializationEnvelope envelope = this.makeEnvelope(METH_NAME);");

		this.emitParameters(operation, parms, pw);

		pw
				.println("       String response=httpTransport.call(METH_NAME, envelope);");
		...
		pw
				.println("      "
						+ this.generateReturnValue(operation, parms, false));

		pw.println("    } catch (Exception e) {");
		pw.println("             this.logError(httpTransport, e);");
		pw.println("           "
				+ this.generateReturnValue(operation, parms, true));
		// return 0;
		pw.println("        }");
		...
		for (int i = 0; i < parms.list.size(); ++i) {
			...
			if (p.getMode() == Parameter.IN) {
				...
				if (GsonUtils.isPrimitiveType(typeName)
						|| GsonUtils.isStringType(typeName))
					pw.println("      envelope.addProperty(\"" + javifiedName
							+ "\"," + javifiedName + ");");
				else if (GsonUtils.isArrayType(typeName)) {
					String baseType = GsonUtils.getBaseType(typeName);
					pw
							.println("     //TODO generate an arraytype  for input array ");
					/**
					 * pw.printf(
					 * "      SoapObject _%s= new SoapObject(this.NAMESPACE,\"%sArray\");\n"
					 * ,javifiedName,baseType); pw.printf(
					 * "     if (%s !=null)      \n" ,javifiedName); // rev
					 * 1.8.4 some arrays of ids may be empty=null in java
					 * pw.printf
					 * ("         for ( Object o : %s) \n",javifiedName);
					...
					pw.println("      envelope.addProperty(\"" + javifiedName
							+ "\"," + javifiedName + ");");// sending an object
															// is not yet
															// implemented
				}
			}
		...
		if (ret != null) {
			...
			if (GsonUtils.isPrimitiveType(typeName))
				...
				if (fault)
					...
					return "return response.toString();";
			} else if (GsonUtils.isArrayType(typeName)) {
				if (fault)
					return "return null;";
				else {
					...
					if (GsonUtils.isPrimitiveType(baseType)) { // array of
																// primitive
																// type

						String wrapperClass = GsonUtils
								.getWrapperClassName(baseType);
						// return "       return null;";
						return "       return GsonUtils.get" + wrapperClass
								+ "Array(response);";
					} else if (GsonUtils.isStringType(baseType)) { // array of
																	// String
						...
						return "Type  collectionType = new TypeToken<Collection<"
								+ baseType
								+ ">>(){}.getType();\n"
								+ "        Collection ret=this.getList(response, collectionType);\n"
								+ "        return ("
								+ baseType
								+ "[]) ret.toArray( new " + baseType + "[0]);";
				...
				// return (ExamenRecordV2)GsonUtils.getObject(response, new
				// ExamenRecordV2());
				String baseType = GsonUtils.getBaseType(typeName);
				if (fault)
					return "return null;";
				else
					return "return (" + baseType
							+ ")GsonUtils.getObject(response,new " + baseType
							+ "());";
			}
		}
