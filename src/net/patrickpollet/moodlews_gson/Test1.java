...
public class Test1 {
	...
	private static final String MOODLE_SERVICE = Constantes.MOODLE_URL
	...
	public Test1() throws Exception {
		...
		Mdl_restserverBindingStub moodle = new Mdl_restserverBindingStub(
				MOODLE_SERVICE, MOODLE_NAMESPACE, false); // Constantes.WS_DEBUG);
		...
		if (lr != null) {
			...
			if (ret != null) {
				...
				CourseRecord[] bis = moodle.get_courses(lr.getClient(), lr
						.getSessionkey(), coursesIds, "id");
				...
				for (int i = 0; i < ret.length; i++) {
					...
					ResourceRecord[] forums = moodle.get_instances_bytype(lr
							.getClient(), lr.getSessionkey(), new String[] { ""
							+ course.getId() }, "id", "forum");
			...
			newU.setUsername("inconnu999");
			newU.setFirstname("inconnu");
			newU.setLastname("inconnu");
			newU.setEmail("inconnu999@patrickpollet.net");
			newU.setIdnumber("inconnu999");
			newU.setPassword("inconnu");
			System.out.println(newU);

			UserRecord[] res = moodle.add_user(lr.getClient(), lr
					.getSessionkey(), newU);
			...
			UserRecord[] users3 = moodle.edit_users(lr.getClient(), lr
					.getSessionkey(), users2);
			for (UserRecord u : users3)
				System.out.println(u);

			// System.out.println (Arrays.toString(users3));

			GradeRecord[] grs = moodle.get_course_grades(lr.getClient(), lr
					.getSessionkey(), "116", "id");
			System.out.println(Arrays.toString(grs));

			GradeItemRecord[] grs2 = moodle.get_module_grades(lr.getClient(),
					lr.getSessionkey(), 98, "quiz", null, "id");
			System.out.println(Arrays.toString(grs2));

			ForumDiscussionRecord[] frd = moodle.get_forum_discussions(lr
					.getClient(), lr.getSessionkey(), 93, 5);
